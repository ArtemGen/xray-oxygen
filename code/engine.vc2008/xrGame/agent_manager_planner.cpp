...
void CAgentManagerPlanner::add_actions		()
{
	...
	action					= new CAgentManagerActionTemplate(&object(), "no_orders", EAgentMngrType::eNoOrders);
	...
	action					= new CAgentManagerActionTemplate(&object(),"gather_items", EAgentMngrType::eGatherItems);
	...
	action					= new CAgentManagerActionTemplate(&object(),"kill_enemy", EAgentMngrType::eKillEnemy);
	add_condition			(action,ePropertyEnemy,				true);
	add_effect				(action,ePropertyEnemy,				false);
	add_operator			(eOperatorKillEnemy,				action);

	action					= new CAgentManagerActionTemplate(&object(),"react_on_danger", EAgentMngrType::eReactOnDanger);
	add_condition			(action,ePropertyEnemy,				false);
